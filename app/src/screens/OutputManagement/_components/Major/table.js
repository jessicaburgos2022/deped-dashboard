    ...
    Swal.fire({
      ...
      if (r.isConfirmed) {
        var ret = await dispatch(
          editOutputStatus({ outputtype: oType, headerid: hId, statusid: sId })
        );
        Swal.fire(
          ret.result,
          ret.message,
      ...
      <Table aria-label="collapsible" className="table table-bordered">
        <TableHead className="thead">
          <TableRow>
            ...
            <TableCell className="interface-table-header">Year</TableCell>
            <TableCell className="interface-table-header">KRA</TableCell>
            <TableCell className="interface-table-header">
              Program / Project
            </TableCell>
            <TableCell className="interface-table-header">Status</TableCell>
            ...
            currentData.map((r) => {
              return (
                <TableRow>
                  <TableCell
                    component="td"
                    className="interface-table-cell  text-center"
                  >
                    {r.Department}
                  </TableCell>
                  <TableCell
                    ...
                    <div style={{ display: "flex", padding: 5 }}>
                      ...
                      {(parseInt(userState.userInfo.acc[0].RoleId) === 1 ||
                        ...
                        <Button
                          ...
                          className="btn btn-secondary"
                          onClick={() =>
                            handleEditOutputStatus(1, r.OutputMajorHeaderId, 2)
                          }
                        >
                          Approve
                        </Button>
